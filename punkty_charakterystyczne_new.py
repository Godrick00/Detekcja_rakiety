...
# -----------------------------
video_path = Path("C://Users/pawel/PycharmProjects/Squash/🇫🇷 Crouin v Cardenas 🇲🇽 ｜ German Open 2025 QFs ｜ FREE Full Match!.mp4").resolve()  #  <-- podaj tu swoją ścieżkę
...
with VideoFileClip(str(video_path)) as clip:
    ...
    end_time = 59 * 60 + 46    # do 59:46 fragment video, który zawiera ujęcie z jednej kamery z widoku od tyłu

    if end_time > duration:
        raise ValueError(f"Koniec fragmentu ({end_time}s) przekracza długość filmu ({duration:.2f}s).")

    selected_length = end_time - start_time
    print(f"Przycinanie filmu od {start_time:.2f}s do {end_time:.2f}s → długość: {selected_length:.2f}s")
...
print(f"Przycięty film zapisano: {trimmed_video_path}")
